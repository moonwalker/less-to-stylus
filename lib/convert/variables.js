...
// Declarations
const replaceDeclarations = ({ variableNamePrefix }) => source => source
  // `@variableName : ` -> `$variableName = `
  .replace(/@([\w|-]+)( *):( *)/g, (_, variableName, spaceBeforeColor, spaceAfterColon) =>
    `$${variableNamePrefix}${variableName}${spaceBeforeColor}=${spaceAfterColon}`)
...
module.exports = (options = { variableNamePrefix: '' }) => source => flow([
  replaceDeclarations,
  replaceNegativeVariables,
  replaceUsage,
  replaceInterpolations,
  replaceDynamicNames,
].map(f => f(options)))(source);
